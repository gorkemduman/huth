defmodule Huth.TokenStore do
  @moduledoc """
  The `Huth.TokenStore` is a simple `GenServer` that manages storage and retrieval
  of tokens `Huth.Token`. When adding to the token store, it also queues tokens
  ...
  @doc ~S"""
  Store a token in the `TokenStore`. Upon storage, Huth will queue the token
  ...
  Retrieve a token from the `TokenStore`.
      ...
      token = %Huth.Token{type:    "Bearer",
                          token:   "123",
                          scope:   "scope",
                          expires: :os.system_time(:seconds) + 3600}
      Huth.TokenStore.store(token)
      {:ok, ^token} = Huth.TokenStore.find(token.scope)
  """
  ...
  defp filter_expired({:ok, %Huth.Token{expires: expires}}, system_time)
